	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"none","width":"64%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('forumstore','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
		<div class="list_bottom">
			<div class="data_box">
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item)" >
						<div class="data_content">
							<div class="data_title">{{item.title}}</div>
							<div class="data_title">发布人：{{item.username}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/forumstoreAdd')
	}
	//判断是否从个人中心跳转
    ...
    router.push(`/index/forumList?fid=${item.fid}`)
		// router.push('forumstoreDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
