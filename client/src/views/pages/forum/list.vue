<template>
	<div class="app-contain" :style='{"width":"64%","padding":"10px 20px","margin":"30px auto 50px","position":"relative","borderRadius":"0px","background":"#fff"}'>
		...
		<el-dialog v-model="detailVisible" :title="''" width="70%" destroy-on-close>
			<div class="forum_detail">
				<div class="forum_title" style="padding: 0 100px;position: relative;">
					{{detailForm.title}}
          <div style="position: absolute;right: 0;top: 0" v-if="storeRecode">
            <el-button @click="storeup" v-if="!storeRecode.id">收藏</el-button>
            <el-button @click="cancelStore" v-else>取消收藏</el-button>
          </div>
				</div>
				<div class="forum_info">
					<span class="forum_name">发布人：{{detailForm.username}}</span>
					<span class="forum_time">发布时间：{{detailForm.addtime}}</span>
          <span class="forum_time">浏览次数：{{detailForm.viewnum||0}}</span>
				</div>
        <div style="display: flex;justify-content: center;column-gap: 20px;">
          <div v-if="hasZan(detailForm)" class="zan" style="cursor: pointer;" @click="toZan(detailForm)">
            <i class="iconfont icon-thumb-up-line2"></i>
            <span>取消赞({{detailForm.zanlist?JSON.parse(detailForm.zanlist).length:0}})</span>
          </div>
          <div v-else-if="hasCai(detailForm)" class="zan" style="cursor: pointer;" @click="toCai(detailForm)">
            <i class="iconfont icon-thumb-down-line2"></i>
            <span>取消踩({{detailForm.cailist?JSON.parse(detailForm.cailist).length:0}})</span>
          </div>
          <template v-else>
            <div class="zan" style="cursor: pointer;" @click="toZan(detailForm)">
              <i class="iconfont icon-thumb-up-line2"></i>
              <span>赞({{detailForm.zanlist?JSON.parse(detailForm.zanlist).length:0}})</span>
            </div>
            <div class="zan" style="cursor: pointer;" @click="toCai(detailForm)">
              <i class="iconfont icon-thumb-down-line2"></i>
              <span>踩({{detailForm.cailist?JSON.parse(detailForm.cailist).length:0}})</span>
            </div>
          </template>
        </div>
				<el-divider />
            ...
            <div style="display: flex;justify-content: space-between;align-items: center">
              <div style="display: flex;justify-content: center;column-gap: 20px;white-space: nowrap;">
                <div v-if="hasZan(item)" class="zan" style="cursor: pointer;" @click="toZan(item)">
                  <i class="iconfont icon-thumb-up-line2"></i>
                  <span>取消赞({{item.zanlist?JSON.parse(item.zanlist).length:0}})</span>
                </div>
                <div v-else-if="hasCai(item)" class="zan" style="cursor: pointer;" @click="toCai(item)">
                  <i class="iconfont icon-thumb-down-line2"></i>
                  <span>取消踩({{item.cailist?JSON.parse(item.cailist).length:0}})</span>
                </div>
                <template v-else>
                  <div class="zan" style="cursor: pointer;" @click="toZan(item)">
                    <i class="iconfont icon-thumb-up-line2"></i>
                    <span>赞({{item.zanlist?JSON.parse(item.zanlist).length:0}})</span>
                  </div>
                  <div class="zan" style="cursor: pointer;" @click="toCai(item)">
                    <i class="iconfont icon-thumb-down-line2"></i>
                    <span>踩({{item.cailist?JSON.parse(item.cailist).length:0}})</span>
                  </div>
                </template>
              </div>
                     ...
                     @click="commentReplyClick(item.id)">回复</div>
						...
						<div class="forum_comment_second_list" v-if="item.childs">
							...
							<div class="forum_comment_second" v-for="(items,indexs) in item.childs" :key="indexs"
								...
								<div class="forum_comment_second_reply_view" style="display: flex;justify-content: space-between;align-items: center">
                  <div style="display: flex;justify-content: center;column-gap: 20px;white-space: nowrap;">
                    <div v-if="hasZan(items)" class="zan" style="cursor: pointer;" @click="toZan(items)">
                      <i class="iconfont icon-thumb-up-line2"></i>
                      <span>取消赞({{items.zanlist?JSON.parse(items.zanlist).length:0}})</span>
                    </div>
                    <div v-else-if="hasCai(items)" class="zan" style="cursor: pointer;" @click="toCai(items)">
                      <i class="iconfont icon-thumb-down-line2"></i>
                      <span>取消踩({{items.cailist?JSON.parse(items.cailist).length:0}})</span>
                    </div>
                    <template v-else>
                      <div class="zan" style="cursor: pointer;" @click="toZan(items)">
                        <i class="iconfont icon-thumb-up-line2"></i>
                        <span>赞({{items.zanlist?JSON.parse(items.zanlist).length:0}})</span>
                      </div>
                      <div class="zan" style="cursor: pointer;" @click="toCai(items)">
                        <i class="iconfont icon-thumb-down-line2"></i>
                        <span>踩({{items.cailist?JSON.parse(items.cailist).length:0}})</span>
                      </div>
                    </template>
                  </div>
...
<script setup>
  ...
  const toZan = (item)=>{
    let arr = item.zanlist?JSON.parse(item.zanlist):[]
    if(!arr.includes(uid.value)){
      arr.push(uid.value)
    }else{
      arr = arr.filter(id=>(id!=uid.value))
    }
    context.$http.post('forum/update',{
      id:item.id,
      zanlist:JSON.stringify(arr)
    }).then(res=>{
      item.zanlist = JSON.stringify(arr)
      context.$message.success("操作成功")
    })
  }
  const toCai = (item)=>{
    let arr = item.cailist?JSON.parse(item.cailist):[]
    if(!arr.includes(uid.value)){
      arr.push(uid.value)
    }else{
      arr = arr.filter(id=>(id!=uid.value))
    ...
    context.$http.post('forum/update',{
      id:item.id,
      cailist:JSON.stringify(arr)
    }).then(res=>{
      item.cailist = JSON.stringify(arr)
      context.$message.success("操作成功")
    })
  }
  const hasZan=(item)=>{
    let zanlist = item.zanlist?JSON.parse(item.zanlist):[]
    if(zanlist.includes(uid.value))return true
  }
  const hasCai = (item)=>{
    let cailist = item.cailist?JSON.parse(item.cailist):[]
    if(cailist.includes(uid.value))return true
  }
  const storeup = ()=>{
    context.$http.post('forumstore/save',{
      avatarurl: detailForm.value.avatarurl,
      fid:detailForm.value.id,
      title:detailForm.value.title,
      uid:uid.value,
      username:detailForm.value.username
    }).then(res=>{
      context.$message.success("收藏成功")
      getStoreup()
    })
  }
  const storeRecode = ref(null)
  const getStoreup = ()=>{
    context.$http.get('forumstore/list',{
      params:{
        uid:uid.value,
        fid:detailForm.value.id,
      }
      ...
      if(res.data.data.list.length){
        storeRecode.value = res.data.data.list[0]
      }else{
        storeRecode.value = {}
      }
    })
  ...
  const cancelStore = ()=>{
    context.$http.post('forumstore/delete',[storeRecode.value.id]).then(res=>{
      context.$message.success("已取消收藏")
      getStoreup()
    })
  }
	//基础信息
	...
	const detailClick = (id = null) => {
        ...
        getStoreup()
        if(detailForm.value.viewnum){
          detailForm.value.viewnum = +detailForm.value.viewnum+1
        }else{
          detailForm.value.viewnum = 1
        }
        context.$http.post('forum/update',{
          id:id,
          viewnum:detailForm.value.viewnum
        })
			})
		}
	}
